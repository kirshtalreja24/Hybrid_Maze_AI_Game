...
def make_enemy_start(maze: Maze) -> tuple:
    """Pick a random walkable cell far from the player (0,0)."""
...
def main():
    ...
    while running:
        ...
        # Rebuild maze if slider changed while not started
        if not started and ui.maze_size != maze.rows:
            regen_maze()
            started = False # keep it unstarted after regen from slider
